...
def create_kafka_consumer(
    topic: str, bootstrap_server: str, offset: str
) -> Type[KafkaConsumer]:
    try:
...
def parse_command_line_arguments():
    ...
    parser.add_argument(
        "--topic_name", default="wikipedia-events", help="Kafka topic name", type=str
    )
    parser.add_argument(
        "--offset",
        default="earliest",
        help="From where to start sonsuming messages",
        type=str,
    )
    parser.add_argument(
        "--mongodb_server", default="localhost", help="MongoDB server", type=str
    )
    parser.add_argument(
        "--mongodb_username", default="root", help="MongoDB username to auth", type=str
    )
    parser.add_argument(
        "--mongodb_password",
        default="password",
        help="MongoDB password to auth",
        type=str,
    )
