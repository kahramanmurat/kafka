...
def parse_command_line_arguments():
    ...
    parser.add_argument(
        "--topic_name",
        default="wikipedia-events",
        help="Destination topic name",
        type=str,
    )
    parser.add_argument(
        "--acks", default="all", help="Kafka Producer acknowledgment", type=str
    )
    parser.add_argument(
        "--events_to_produce",
...
if __name__ == "__main__":
    ...
    for event in SSEClient(url):
        ...
        if messages_count >= args.events_to_produce:
            print(
                f"Producer will be killed as {args.events_to_produce} events were producted"
            )
            exit(0)
